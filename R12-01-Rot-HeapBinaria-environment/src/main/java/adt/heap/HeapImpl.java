...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
        ...
        if (left <= index && comparator.compare(heap[left], heap[best]) > 0) best = left;
        if (right <= index && comparator.compare(heap[right], heap[best]) > 0) best = right;

        if (best != position) {
            Util.swap(heap, position, best);
            heapify(best);
        }
    }
	...
	public T[] heapsort(T[] array) {
		...
		if (!(array == null || array.length == 0)){
			...
			heap = originalHeap;
			index = originalIndex;
			setComparator(originalComparator);
		}
