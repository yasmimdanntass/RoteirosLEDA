...
public class StudentMinHeapTest {
	...
	public void testOrderStat() {
		OrderStatistics os = new OrderStatisticsHeapImpl();

		assertEquals(2,
				os.getOrderStatistics(new Integer[] { 3, 2, 1}, 2));

		assertEquals(null,
				os.getOrderStatistics(new Integer[] {}, 2));

		assertEquals(null,
				os.getOrderStatistics(new Integer[] { 3, 2, 1}, 5));
	}

	@Test
	public void testFS() {
		FloorCeilHeap os = new FloorCeilHeapImpl(new ComparatorMaxHeap<Integer>());

		assertEquals((Integer) 2,
				os.ceil(new Integer[] { 3, 2, 1}, 2));

		assertEquals((Integer) null,
				os.ceil(new Integer[] { 3, 2, 1}, 4));

		assertEquals((Integer) null,
				os.floor(new Integer[] { 3, 2, 1}, 0));

		assertEquals((Integer) 2,
				os.floor(new Integer[] { 3, 2, 1}, 2));

		assertEquals((Integer) null,
				os.floor(new Integer[] { 3, 2, 1}, 0));

		assertEquals((Integer) 3,
				os.floor(new Integer[] { 3, 2, 1}, 5));
