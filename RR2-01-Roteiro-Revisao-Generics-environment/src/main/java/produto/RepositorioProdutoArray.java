 ...
 * @author Adalberto
 */
public class RepositorioProdutoArray<T extends Produto> {
	...
	public RepositorioProdutoArray(int size) {
		this.produtos = (T[]) new Produto[size]; 
	}
	...
	public void remover(int codigo) {
		...
		if (pos != -1) {
			produtos[pos] = produtos[index]; // move o último para a posição removida
			produtos[index] = null;
			index--;
		} else {
			throw new RuntimeException("PRODUTO NÃO ENCONTRADO!");
